...
exports.deleteOne = Model => catchAsync(async (req, res, next) => { // Model is an argument, which returns another function // (which document you want to delete the Model from)
    ...
    let filterObj = {}
    if (Model.modelName === "Dessert") { // defined modelName in model Object ("Tag", "Dessert", "Review")
...
exports.updateOne = Model => catchAsync(async (req, res, next) => {
        ...
        if (Model.modelName === "Dessert") { // defined modelName in model Object ("Tag", "Exp", "Review")
            filterObj._id = req.params.eid;
            filterObj.host = req.user._id
            allows = ['name', 'description', 'pictureUrl', 'price', 'tags']
            if (req.body.tags) {
